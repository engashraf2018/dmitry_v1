...
<th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">مرسلة إلي</th>
...
$sql = "SELECT SQL_CALC_FOUND_ROWS * from global_message where id_sender='$prifix' order by id desc limit $start,$per_page";
...
$subject=$row['subject'];
$date=$row['date'];
//$to=substr($row['id_reciver'], 1);

$ids = substr($row['id_reciver'], 1);
$to=$row['id_reciver'][0];
...
<td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;"><?php echo $to." ( $user ) ";?></td>
<td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;"><?php echo $view;?></td>
<td class="center">
<a href="read_messages_send_m?id_messages=<?php echo $id_messages?>" class="btn btn-sm btn-success">
<i class="fa fa-eye"></i>
</a>
...
{
$pr_page=$page-1;
echo '<a href="send_messages?r='.$count.'&page='.$pr_page.'&shop_id='.$id_admin.'">«'.$prev.'</a>';							  
}
else
{
echo '<span>«'.$prev.'</span>';
...
{
echo '<a href="send_messages?r='.$count.'&page='.$i.'&shop_id='.$id_admin.'">'.$i.'</a>';
}
}
