<?php

$base_url=base_url();

if(!isset($_SESSION['admin_name'])||$_SESSION['admin_name']==""){

header("Location:".$dd."home/login");  

}

else{
$id_admin=$_SESSION['id_admin'];
$admin_name=$_SESSION['admin_name'];
$last_login=$_SESSION['last_login'];	
}
$no_activite="غير مفعل";
$activite="مفعل";
?>



<!DOCTYPE html>



<!--[if !IE]><!--><html class="sidebar sidebar-discover"><!-- <![endif]-->

<head>

<title>السائقين</title>
<meta charset="utf-8">
<?php include ("home/inc/head1.inc");?>
<script src="<?php echo $base_url?>home/js/jquery-1.7.2.min.js"></script>
<script>

$(document).ready(function(e) {

    $("#checkAll").change(function(){

		$("input[type=checkbox]").not("#checkAll").each(function() {

            this.checked=!this.checked;

        });

	});

	$(".cancel").click(function(){

		if($('input[type=checkbox]:not("#checkAll"):checked').length>0){

			$('#form').unbind('submit').submit();//renable submit

		}

	    else{

		    alert("اختر على الاقل رابط واحد للحذف");

	}

			

	});

	

});

</script>

<script>

$(document).ready(function(e) {
$(".change").click(function(e) {
var id = $(this).data("id");
var data={id:id};
			$.ajax({
				url: '<?php echo base_url("home/check_view_drivers") ?>',
                type: 'POST',
                data: data,				
                success: function( data ) {
                	if (data == "1") {
                		$(".views-"+id).text("مفعل");
                		$(".views-"+id).css("color","green");
                	}
                	if (data == "0") {
                		$(".views-"+id).text("غير مفعل");
                		$(".views-"+id).css("color","red");
                	}
				}
         });
	});
});


$(document).ready(function(e) {
$(".edit").click(function(e) {
var id = $(this).data("id");
var data={id:id};
			$.ajax({
				url: '<?php echo base_url("home/check_view_code_actvation") ?>',
                type: 'POST',
                data: data,				
                success: function( data ) {
                	if (data == "1") {
                		$(".code_actvation-"+id).text("مفعل");
                		$(".code_actvation-"+id).css("color","green");
                	}
                	if (data == "0") {
                		$(".code_actvation-"+id).text("غير مفعل");
                		$(".code_actvation-"+id).css("color","red");
                	}
				}
         });
	});
});


</script>



<script>

$(document).ready(function(e) {

      $(".add").click(function(e) {

        window.location.assign("<?php echo $base_url?>home/add_article");

    });



});

  

</script>

</head>

<body class="">

<div class="container-fluid menu-hidden">

<?php 

include ("home/inc/sidebar.inc");

?>

</div>

<div id="content">

<?php 

include("home/inc/header.inc");

if(isset($_GET['success'])){

?>

<script>

$(document).ready(function(e) {

 msg_success("تم تنفيذ المطلوب بنجاح");

});

</script>

<?php }?>		

<div class="alert-msgs"></div>

<div class="innerLR">

	<div class="widget">

		<div class="widget-body">
<form method="post" action="<?php echo base_url("home/search_articles");?>">		
<input type="text" name="search">
<input type="submit" value="ابحث">
</form>
<table class="dynamicTable scrollVertical table table-primary">

	<thead>

		<tr>

        <th scope="col" class="center"><input type="checkbox" id="checkAll"></th>

            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">التليفون</th>
            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">الايميل</th>
            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">النوع</th>
            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">الخدمة</th>
            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">تاريخ </th>
            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">الصورة</th>
             <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">الحالة</th>
            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">التوثيق</th>
            <th class="center" style="font-family: 'Droid Arabic Kufi', sans-serif; font-weight:400;">الرصيد</th>
		</tr>
	</thead>
<tbody>

<form action="<?php echo $base_url?>home/delete_articles" method="POST" id="form">

<?php

						?>

                         <?php if(count($results)==0){?>

<tr style="height:70px">

<td colspan="8" class="center" style="color:#E63998; font-size:18px">لا يوجد بيانات حاليا </td>

</tr>

                          <?php }?>

                        <?php
foreach($results as $jobs){
$id_categ=$jobs->id;
 $view=$jobs->view;
 $code_actvation=$jobs->code_actvation;
 
 $image=$jobs->img;
 if($image!=""){
 $img="../../site/ar/images/driver/".$image;    
 }
 else {
$img="../../site/ar/images/driver/defaultmain.png";    
 }
							  switch($view){
								  case 0:
								    $view="<span class='view' style='color:red'>$no_activite</span>";
								    break;
								  case 1:
								    $view="<span class='view' style='color:green'>$activite</span>";
								    break;
								  default:
								    break; 
							  }
							  
							  switch($code_actvation){
								  case 0:
								    $code_actvation="<span class='view' style='color:red'>$no_activite</span>";
								    break;
								  case 1:
								    $code_actvation="<span class='view' style='color:green'>$activite</span>";
								    break;
								  default:
								    break; 
							  }
$date=date("Y-m-d",strtotime($jobs->date));

$attachment = $this->db->get_where("attachment",array("id_clients"=>$id_categ))->result();
$company = $this->db->get_where("company",array("id_clients"=>$id_categ))->result();
if(count($attachment)==1){
$account_type="فردى";
foreach($attachment as $attachment){
	$id_category=$attachment->id_category;
	$result_category = $this->db->get_where("category",array("id"=>$id_category))->result();
	 foreach($result_category as $result_category){
		 $name_category=$result_category->titleeng;
	 }
}
}
if(count($company)==1){
$account_type="شركات";
foreach($company as $company){
$id_category=$company->id_category;
$result_category = $this->db->get_where("category",array("id"=>$id_category))->result();
	 foreach($result_category as $result_category){
		 $name_category=$result_category->titleeng;
	 }
	}
}
?>



                        <tr>
<td class="center"><input type="checkbox" name="check[]" value="<?php echo $id_categ;?>"></td>
<td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;"><?php
echo character_limiter($jobs->phone,50);?></td>
<td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;"><?php echo $jobs->email ;?></td>
<td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;"><?php echo $account_type ;?></td>

<td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;"><?php echo $name_category;?></td>
<td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;"><?php echo $date;?></td>
 <td class="center" style="font-family: 'Droid Arabic Kufi', sans-serif;">
 <a class="example-image-link" href="<?php echo $img;?>" data-lightbox="example-1">مشاهدة</a></td>
<td class="center ">
<div class="code_actvation-<?php echo $id_categ;?>" <?php if ($code_actvation== "مفعل") {echo "style='color:green'";}
else{echo "style='color:red'";} ?> style="font-family: 'Droid Arabic Kufi', sans-serif;">
<?php if ($code_actvation == "مفعل") {echo $code_actvation;}else{echo $code_actvation;} ?>
</div>

    <a href="#" data-id="<?php echo $id_categ;?>" class="table-icon edit" title="تغيير الحالة"></a>
    </td>
    <td class="center">
    <div class="views-<?php echo $id_categ;?>" <?php if ($view == "مفعل") {echo "style='color:green'";}
	else{echo "style='color:red'";	} ?> style="font-family: 'Droid Arabic Kufi', sans-serif;">
   <?php if ($view == "مفعل") {echo $view;}else{echo $view;} ?>
   </div>
    <a href="#" data-id="<?php echo $id_categ;?>" class="table-icon change" title="تغيير الحالة"></a>
    </td>

<td class="center">
<!--<a href="<?php echo $base_url?>home/delete_articles?id=<?php echo $id_categ;?>" class="btn btn-sm btn-danger"><i class="fa fa-times"></i></a>
<a href="<?php echo $base_url?>home/update_article?id=<?php echo $id_categ;?>" class="btn btn-sm btn-success"><i class="fa fa-pencil"></i></a>-->

							</td>
						</tr>

                        <?php

						  }

						?>

                         </form>	

					</tbody>       

				</table>

                <?php foreach($links as $link){?><?php echo $link;?><?php } ?>



<div style="text-align:center">

               <?php

			   if(count($results)!=0){

			   ?>

<button class="cancel Go">حذف</button>	

               <?php

			   }else{

			   echo '<script>$("#checkAll").attr("disabled","disabled");</script>';

			   }

			   ?>

<button class="add Go">اضافة</button>

       </div>

</div>

	</div>

</div>

<div class="clearfix"></div>

<div id="footer" class="hidden-print">

<?php

 include ("home/inc/footer.inc");

?>

</div>

</div>

	<?php



    include ("home/inc/headf1.inc");



	?>

</body>

</html>