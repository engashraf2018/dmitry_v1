...
class lib_pagination{
   ...
   function __construct() {
    ...
    function create_pagination($pg_config) {
        ...
        $config['first_tag_open']  = '<button type="button" class="btn green">';  //The opening tag for the "first" link.
        $config['first_tag_close'] = '</button>';//The closing tag for the "first" link.
        ...
        $config['last_tag_open']   = '<button type="button" class="btn green">';   //The opening tag for the "last" link.
        $config['last_tag_close']  = '</button>'; //The closing tag for the "last" link.
        ...
        $config['next_tag_open']   = '<button type="button" class="btn blue">';   //The opening tag for the "next" link.
        $config['next_tag_close']  = '</button>'; //The closing tag for the "next" link.
        ...
        $config['prev_tag_open']   = '<button type="button" class="btn blue">';   //The opening tag for the "previous" link.
        $config['prev_tag_close']  = '</button>'; //The closing tag for the "previous" link.
        
        $config['cur_tag_open']    = '<li type="button" class="btn red">';      //$config['cur_tag_open'] = '<button>';
        $config['cur_tag_close']   = '</button>';    //$config['cur_tag_close'] = '</b>';
        
        $config['num_tag_open']    = '<button type="button" class="btn green">';    //The opening tag for the "digit" link.
        $config['num_tag_close']   = '</button>';  //The closing tag for the "digit" link.
        */

        $config['first_link']      = 'First';      //The text you would like shown in the "first" link on the left. If you do not want this link rendered, you can set its value to FALSE.
        $config['first_tag_open']  = '<li>';  //The opening tag for the "first" link.
        $config['first_tag_close'] = '</li>';//The closing tag for the "first" link.
        ...
        $config['cur_tag_open']    = '<li class="active"><a href="javascript:;">';      //$config['cur_tag_open'] = '<li>';
        $config['cur_tag_close']   = '</a></li>';    //$config['cur_tag_close'] = '</b>';
        ...
        //echo $last_segment;die;
        // if (strpos($last_segment,'?')==true)
        //     {
        //         $new = explode("?", $last_segment);
        //         $config["base_url"] = $this->get_current_url_without_pagenum()."/".$new[0]."/?".$new[1];
